...
public class GraphicsDisplay1 extends JPanel {
    ...
    private ArrayList<Double[]> graphicsData;//Двумерный массив координат точек,используется многими методами
    ...
    //вызывается при загрузке файла, рисует наш график
    public void showGraphics(ArrayList<Double[]> graphicsData) {
        ...
        //масштабирование(приближение графика)
    ...
    //перерисовка при изменении любого параметра
    ...
    protected void paintGraphics(Graphics2D canvas) {
        ...
        for (Double[] point : this.graphicsData) {
            if ((currentX != null) && (currentY != null)) {
                canvas.draw(new Line2D.Double(xyToPoint(currentX + 0.1, currentY + 0.1),
                        xyToPoint(point[0] + 0.1, point[1] + 0.1)));
            }
            currentX = point[0];
            currentY = point[1];
    ...
    protected void paintAxis(Graphics2D canvas) {
        ...
        if (minX <= 0.0 && maxX >= 0.0) {
            ...
            Point2D.Double labelPosOne = xyToPoint(0.1, 0);
            Point2D.Double labelShift = xyToPoint(0, 0);

            canvas.drawString("y", (float) labelPos.getX() + 10, (float) (labelPos.getY() - bounds.getY()));
            canvas.drawString("1", (float) labelPosOne.getX() + 10, (float) (labelPosOne.getY() - bounds.getY()));
            GeneralPath Scale = new GeneralPath();
            Point2D.Double center = xyToPoint(0, -4);
            Scale.moveTo(center.x, center.y);
            Scale.lineTo(center.x - 7, center.y);
            Scale.lineTo(center.x + 7, center.y);
            Scale.closePath();
            canvas.draw(Scale);
        ...
        Point2D.Double labelPosZero = xyToPoint(minX, maxY + 0.1);
    ...
    private void paintLabels(Graphics2D canvas) {
        ...
        if (selectedMarker >= 0) {
            Point2D.Double point = xyToPoint(((Double[]) graphicsData.get(selectedMarker))[0] + 0.1,
                    ((Double[]) graphicsData.get(selectedMarker))[1] + 0.1);
    ...
    protected int findSelectedPoint(int x, int y) {
        ...
        int pos = 0;
        for (Double[] point : graphicsData) {
            Point2D.Double screenPoint = xyToPoint(point[0] + 0.1, point[1] + 0.1);
    ...
    public class MouseMotionHandler implements MouseMotionListener {
        ...
        public void mouseDragged(MouseEvent ev) {
            if (changeMode) {
                //Добавить поворот (при нажатии на маркер)
